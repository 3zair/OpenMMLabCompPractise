...
def split_data(dataset_path, target_path, train_set_rate=0.8):
    count = 0
    data = {}
    classes = []
    ...
    for name in os.listdir(dataset_path):
        ...
        for file_name in os.listdir(dir_path):
            ...
            try:
                ...
                raise ('no file path:', file_path)
            count += len(data[name])
            print("{} :{}".format(name, len(data[name])))
    print("total of image :{}".format(count))

    for i, (cls_name, img_list) in enumerate(data.items()):
        img_data_size = len(img_list)
        random_index = np.random.choice(img_data_size, img_data_size, replace=False)
        train_size = int(img_data_size * train_set_rate)
        train_data_index = random_index[:train_size]
        val_data_index = random_index[train_size:]

        train_data_path = os.path.join(target_path, "train", cls_name)
        val_data_path = os.path.join(target_path, "val", cls_name)
        ...
        write_file(classes, os.path.join(target_path, "classes.txt"))
        write_file(train_dataset, os.path.join(target_path, "train.txt"))
        write_file(val_dataset, os.path.join(target_path, "val.txt"))
...
def main():
    ...
    split_data(src_data_path, target_data_path, 0.8)
